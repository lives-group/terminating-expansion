...
{-- 
Theorem 
Change fix name only changes name
--}
termUnchangedOnFix : ∀{t : Term}(v : Name) → NamedFun t → NamedFun (changeFixName t v)
termUnchangedOnFix {.(fix v₁ _)} v (funName v₁) = funName v